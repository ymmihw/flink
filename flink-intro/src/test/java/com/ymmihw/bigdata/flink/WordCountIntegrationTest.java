...
public class WordCountIntegrationTest {
  ...
  public void givenDataSet_whenExecuteWordCount_thenReturnWordCount() throws Exception {
    // given
    List<String> lines =
        Arrays.asList("This is a first sentence", "This is a second sentence with a one word");
    ...
    assertThat(collect).containsExactlyInAnyOrder(new Tuple2<>("a", 3), new Tuple2<>("sentence", 2),
        new Tuple2<>("word", 1), new Tuple2<>("is", 2), new Tuple2<>("this", 2),
        new Tuple2<>("second", 1), new Tuple2<>("first", 1), new Tuple2<>("with", 1),
        new Tuple2<>("one", 1));
  }

  @Test
  public void givenListOfAmounts_whenUseMapReduce_thenSumAmountsThatAreOnlyAboveThreshold()
      throws Exception {
    // given
    ...
    List<Integer> collect =
        amounts.filter(a -> a > threshold).reduce((integer, t1) -> integer + t1).collect();
  ...
  @Test
  public void givenDataSetOfComplexObjects_whenMapToGetOneField_thenReturnedListHaveProperElements()
      throws Exception {
    // given
    DataSet<Person> personDataSource =
        env.fromCollection(Arrays.asList(new Person(23, "Tom"), new Person(75, "Michael")));
  ...
  public void givenDataSet_whenSortItByOneField_thenShouldReturnSortedDataSet() throws Exception {
    ...
    DataSet<Tuple2<Integer, String>> transactions =
        env.fromElements(fourthPerson, secondPerson, thirdPerson, firstPerson);

    // when
    List<Tuple2<Integer, String>> sorted =
        transactions.sortPartition(new IdKeySelectorTransaction(), Order.ASCENDING).collect();
  ...
  public void giveTwoDataSets_whenJoinUsingId_thenProduceJoinedData() throws Exception {
    ...
    List<Tuple2<Tuple2<Integer, String>, Tuple3<Integer, String, String>>> joined =
        transactions.join(addresses).where(new IdKeySelectorTransaction())
            .equalTo(new IdKeySelectorAddress()).collect();
  ...
  @Test
  public void givenStreamOfEvents_whenProcessEvents_thenShouldPrintResultsOnSinkOperation()
      throws Exception {
    // given
  ...
  @Test
  public void givenStreamOfEvents_whenProcessEvents_thenShouldApplyWindowingOnTransformation()
      throws Exception {
    // given
    final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
