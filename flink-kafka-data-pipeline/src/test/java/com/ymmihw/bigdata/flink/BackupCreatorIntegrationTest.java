...
public class BackupCreatorIntegrationTest {
  ...
  @Test
  public void givenProperJson_whenDeserializeIsInvoked_thenProperObjectIsReturned()
      throws IOException {
    ...
    DeserializationSchema<InputMessage> deserializationSchema =
  ...
  @Test
  public void givenMultipleInputMessagesFromDifferentDays_whenBackupCreatorIsUser_thenMessagesAreGroupedProperly()
      throws Exception {
    LocalDateTime currentTime = LocalDateTime.now();
    InputMessage message = new InputMessage("Me", "User", currentTime, "First TestMessage");
    InputMessage secondMessage =
        new InputMessage("Me", "User", currentTime.plusHours(1), "First TestMessage");
    InputMessage thirdMessage =
        new InputMessage("Me", "User", currentTime.plusHours(2), "First TestMessage");
    InputMessage fourthMessage =
        new InputMessage("Me", "User", currentTime.plusHours(3), "First TestMessage");
    InputMessage fifthMessage =
        new InputMessage("Me", "User", currentTime.plusHours(25), "First TestMessage");
    InputMessage sixthMessage =
        new InputMessage("Me", "User", currentTime.plusHours(26), "First TestMessage");

    List<InputMessage> firstBackupMessages =
        Arrays.asList(message, secondMessage, thirdMessage, fourthMessage);
    ...
    testDataSet.assignTimestampsAndWatermarks(new InputMessageTimestampAssigner())
        .timeWindowAll(Time.hours(24)).aggregate(new BackupAggregator()).addSink(sink);
  ...
  @Test
  public void givenProperBackupObject_whenSerializeIsInvoked_thenObjectIsProperlySerialized()
      throws IOException {
    InputMessage message = new InputMessage("Me", "User", LocalDateTime.now(), "Test Message");
  ...
  private static class CollectingSink implements SinkFunction<Backup> {
    ...
    @Override
    public synchronized void invoke(Backup value, @SuppressWarnings("rawtypes") Context context)
        throws Exception {
      backups.add(value);
    }
  }
